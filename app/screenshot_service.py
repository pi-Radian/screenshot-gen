...
async def get_screenshot_from_url(url: str):
    ...
    driver_name = (
        ...
        else "chromedriver_linux64"
    )
    driver_executable = f"/drivers/{driver_name}/chromedriver"
    driver_executable = os.getcwd() + driver_executable
    driver_executable = (
        driver_executable if platform.system() == "Darwin" else "/usr/bin/chromedriver"
    )
    print(driver_executable)
    driver = webdriver.Chrome(options=options, executable_path=driver_executable)
